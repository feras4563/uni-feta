...
export const TreeProvider = ({
  ...
  const currentExpandedSet = isExpandedControlled
  ...
  const toggleExpanded = useCallback((nodeId: string) => {
    if (isExpandedControlled) {
      const currentArray = Array.from(currentExpandedSet);
      const newArray = currentExpandedSet.has(nodeId)
        ? currentArray.filter(id => id !== nodeId)
        : [...currentArray, nodeId];
      onExpandedChange?.(newArray);
    } else {
  ...
  }, [isExpandedControlled, currentExpandedSet, onExpandedChange]);
...
export const TreeNodeTrigger = ({
  ...
  return (
    <motion.div
      className={cn(
        'relative flex cursor-pointer items-center rounded-md px-2 py-1.5 transition-all duration-200',
        'hover:bg-gray-100',
      ...
      style={{ paddingRight: level * (indent ?? 0) + 8 }}
...
export const TreeExpander = ({
  ...
  if (!hasChildren) {
    return <div className="mr-1.5 h-4 w-4" />;
  }
  ...
  return (
    <motion.div
      ...
      className={cn(
        'mr-1.5 flex h-4 w-4 cursor-pointer items-center justify-center',
        className
      )}
      onClick={(e) => {
        e.stopPropagation();
...
export const TreeIcon = ({
  ...
  return (
    <motion.div
      className={cn(
        'mr-2 flex h-4 w-4 items-center justify-center text-gray-500',
        className
      )}
...
export const TreeLabel = ({ className, ...props }: TreeLabelProps) => (
  <span className={cn('flex-1 truncate text-sm font-normal', className)} {...props} />
);
