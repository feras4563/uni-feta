...
export default function AddJournalEntry() {
  ...
  const loadAccounts = async () => {
    try {
      ...
      // Filter out group accounts - only detail accounts can have transactions
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert('تم حفظ القيد كمسودة بنجاح\n\nيمكنك تعديله في أي وقت أو ترحيله للدفتر العام.');
      navigate(`/finance/journal-entry/${response.data.id}`);
    } catch (error: any) {
      console.error('Error saving journal entry:', error);
      alert('خطأ في حفظ القيد: ' + (error.message || 'حدث خطأ غير متوقع'));
    } finally {
      setLoading(false);
    }
  };
        ...
        <div className="px-4 sm:px-6 lg:px-8">
          <div className="flex h-16 justify-between items-center">
            <div className="flex items-center">
              ...
              <h1 className="text-xl font-semibold text-gray-900">قيد يومية جديد</h1>
            </div>
                ...
                <table className="w-full">
                  <thead>
                    <tr className="border-b border-gray-200 bg-gray-50">
                      ...
                      <th className="px-4 py-3 text-right text-sm font-medium text-gray-700">نوع الحركة</th>
                    ...
                    {lines.map((line, index) => (
                      <tr key={line.id} className="border-b border-gray-100 hover:bg-gray-50">
                        ...
                        <td className="px-4 py-3 text-sm text-gray-500">
                          {line.debit ? 'مدين' : line.credit ? 'دائن' : '-'}
                        </td>
                        <td className="px-4 py-3">
                          <input
                            type="number"
