...
import { formatCurrency, formatDate, formatPercent, toLatinDigits } from '../../lib/utils';
              ...
              >
                <div className="flex items-center justify-between">
                  <div className="flex items-center">
                    ...
                    <div>
                      <h3 className="font-bold text-gray-900">{toLatinDigits(invoice.invoice_number)}</h3>
                      <div className="flex items-center space-x-3 space-x-reverse text-xs text-gray-500 mt-1">
                        ...
                        {invoice.invoice_date && <span><i className="fas fa-clock ml-1"></i>{formatDate(invoice.invoice_date)}</span>}
                      </div>
                    </div>
                  </div>
                  <div className="flex items-center space-x-4 space-x-reverse">
                    ...
                    <div className="text-left">
                      <p className="text-lg font-bold text-gray-900">{formatCurrency(invoice.total_amount, 'دينار')}</p>
                      {invoice.balance > 0 && (
                        <p className="text-xs text-red-600">متبقي: {formatCurrency(invoice.balance, 'دينار')}</p>
                      )}
                    </div>
                ...
                {invoice.total_amount > 0 && (
                  <div className="mt-4">
                    ...
                    <div className="flex justify-between text-xs text-gray-500 mt-1">
                      <span>مدفوع: {formatCurrency(invoice.paid_amount, 'دينار')}</span>
                      <span>{formatPercent(Math.round((invoice.paid_amount / invoice.total_amount) * 100))}</span>
                    </div>
                  </div>
                )}
              </div>
                      ...
                      <tbody>
                        {invoice.items.map((item: any, idx: number) => (
                          <tr key={idx} className="border-b border-gray-100">
                            ...
                            <td className="py-2 text-left font-medium text-gray-900">{formatCurrency(item.amount, 'دينار')}</td>
                          </tr>
                        ))}
                      </tbody>
                      <tfoot>
                        {invoice.discount > 0 && (
                          <tr className="border-t border-gray-200">
                            ...
                            <td className="py-2 text-left text-green-700 font-medium">-{formatCurrency(invoice.discount, 'دينار')}</td>
                          </tr>
                        )}
                        <tr className="border-t-2 border-gray-300">
                          <td colSpan={2} className="py-2 font-bold text-gray-900">الإجمالي</td>
                          <td className="py-2 text-left font-bold text-gray-900">{formatCurrency(invoice.total_amount, 'دينار')}</td>
                        </tr>
                      </tfoot>
                    </table>
                  </div>
